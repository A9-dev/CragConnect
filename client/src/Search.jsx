...
function Search() {
  ...
  const handleSubscribe = (toSubscribeUsername) => {
    ...
    userList.find((user) => user.username === toSubscribeUsername).subscribers.push(username);

    refreshFollowingFeed();
  };

  const handleUnsubscribe = (toUnsubscribeUsername) => {
    setSubscriptions(subscriptions.filter((sub) => sub !== toUnsubscribeUsername));
    unsubscribe(username, toUnsubscribeUsername);
    userList.find((user) => user.username === toUnsubscribeUsername).subscribers.pop(username);
    refreshFollowingFeed();
  };
              ...
              <CardBody>
                ...
                {loggedIn &&
                  (subscriptions && subscriptions.includes(user.username) ? (
                    <Button ml={2} width="100%" onClick={() => handleUnsubscribe(user.username)}>
                      Unfollow
                    </Button>
                  ) : (
                    <Button ml={2} width="100%" onClick={() => handleSubscribe(user.username)}>
                      Follow
                    </Button>
                  ))}
              </CardBody>
